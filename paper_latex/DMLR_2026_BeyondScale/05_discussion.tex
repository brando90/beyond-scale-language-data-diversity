...
While our controlled experiments show a consistent positive trend between diversity and evaluation performance across 33 models (Figure~\ref{fig:div_vs_val_ce}), we acknowledge important caveats.
First, the diversity range in our interventional experiments is narrow (0.158--0.195), as it is bounded by available training corpora in the specialized domains (patents and medical abstracts).
Extending these experiments to larger diversity ranges with more diverse web-scale datasets would strengthen the conclusions.
Second, the relationship between diversity and downstream benchmark accuracy (e.g., MMLU) is weaker than the diversity--cross-entropy relationship, because benchmark scores are noisier and small domain-specific models perform near random on knowledge-intensive tasks.
We therefore present our diversity$\to$performance claim primarily through cross-entropy evaluation, with benchmark results as supporting evidence.
