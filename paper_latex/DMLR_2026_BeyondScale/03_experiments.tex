...
To instill further confidence in the diversity coefficient, we perform a correlation analysis with data distributional properties on the GINC dataset synthetic language dataset \citep{ginc}.
GINC generates sequences by modeling how real documents are generated given a fixed number of latent document concepts through a mixture of Hidden Markov Models (HMM), where each HMM has a latent concept that models document statistics, e.g. wiki bio.
...
\subsection{GPT-4 Annotation Validates Diversity Metric Rankings} \label{sec:gpt4_validation}
To further validate that the diversity coefficient captures human-recognizable diversity, we conduct a GPT-4 annotation study as a scalable proxy for human evaluation.

\textbf{Experiments:}
We sampled 30 pairs of text batches (8 samples each) from datasets with known different diversity coefficients: C4 (0.208), WikiText-103 (0.207), OpenWebText (0.199), and BookCorpus (0.160).
For each pair, we presented both batches to GPT-4 (in randomized order to prevent position bias) and asked: ``Which collection exhibits greater topical and stylistic diversity?''
We then compared GPT-4's judgment with the Task2Vec diversity coefficient ranking.

\textbf{Results:}
GPT-4 agreed with the Task2Vec ranking in 24 out of 29 valid comparisons (\textbf{82.8\% agreement}, Cohen's $\kappa = 0.655$, indicating substantial agreement).
Agreement was strongest when the diversity gap was large: pairs involving BookCorpus (div=0.160) versus web-scale datasets achieved 100\% agreement.
Disagreements occurred primarily between datasets with very similar diversity coefficients (e.g., OpenWebText 0.199 vs.\ C4 0.208), where both collections are drawn from diverse web text and the distinction is subtle.
These results provide evidence that the Task2Vec diversity coefficient captures a notion of diversity that aligns with an intelligent observer's judgment.

\subsection{Controlling for Confounding Factors} \label{sec:confounding}
A natural concern is that merging datasets changes more than just diversity---it may also change vocabulary overlap with evaluation data or domain coverage.
We conduct two ablations to isolate diversity's effect.

\textbf{Vocabulary overlap ablation.}
We compute token-level Jaccard similarity between each training set (USPTO, PubMed, USPTO+PubMed) and evaluation sets (C4, OpenWebText2) using 5,000 samples per dataset.
While type-level Jaccard (unique token overlap) trivially increases for the combined dataset due to larger vocabulary coverage, the frequency-weighted Jaccard actually \emph{decreases} (e.g., 0.364 $\to$ 0.281 for C4).
This means the token frequency distribution of the combined dataset is \emph{less similar} to the evaluation data than either individual dataset, ruling out vocabulary overlap as a confounding factor for the performance improvements.

\textbf{Domain control ablation.}
Neither USPTO (patents) nor PubMed (medical abstracts) is a domain match for our evaluation benchmarks (C4, OpenWebText2, MMLU).
Yet the combined dataset (div=0.195) consistently achieves the best cross-entropy on diverse evaluation sets (Figure~\ref{fig:div_vs_val_ce}).
On MMLU, the average accuracy across model families follows the diversity ranking: USPTO (0.232), PubMed (0.234), USPTO+PubMed (0.235).
Since no training domain matches the evaluation domain, the performance improvement is attributable to diversity rather than domain overlap.
