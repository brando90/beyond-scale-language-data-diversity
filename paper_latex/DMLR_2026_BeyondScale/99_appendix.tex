...
We will provide a rephrasing from the original Task2Vec \citep{task2vec} explaining why Task2Vec is captures the importance of the weights and therefore why it serves as a unique fingerprint for a task.
...
% Other approaches to data-centric Machine Learning involve efficiently cleaning datasets by pruning pre-training datasets through the discard of weak examples, aiming to disrupt the power-law relationship between loss and performance \citep{abbas2024effective} \citep{}
...
This last condition is crucial for CKA, PWCCA distances to not be vacuous \citep{svcca, curse_low_div}. 
...
% In this section we will give our review of related work by \citep{Achille_dist_tasks, task2vec} to define distances between task, complexity, embeddings of tasks and related concepts.
% \subsubsection{Static Distance between tasks}
% To define the static distance between tasks \citep{Achille_dist_tasks} first define the complexity of a task then define the static distance using the complexity of a task.
% \textbf{Task:} \citep{Achille_dist_tasks} define a task $\tau$ as the r.v. $y$ one is trying to infer given $x$ or the representing data set for the random variables with a loss function.
...
% This function is also called the Kolmogrov Structure Function \citep{Achille_dist_tasks}.
...
% Intuitively, this measure the cost of encoding $\tau$ (or the data set) using model $m$ when the cost the model $k(m)$ is discounted by a factor of $\beta$ \citep{Achille_dist_tasks}.
...
\textbf{C4}, a 305GB cleaned version of Common Crawl's web crawl corpus in English \citep{C4}. Sequences in C4 were extracted from the web via de-duplication methods and heuristics to remove boiler-plate and gibberish.

\textbf{WikiText-103}, a 500MB collection of over 100 million tokens extracted from the set of verified Good and Featured articles on Wikipedia \citep{wikitext}. 

\textbf{The Pile}, a 825 GiB open-source English-text corpus for language modeling that combines 22 smaller, high-quality datasets from diverse sources \citep{thepile}. These sources include Pile-CC (Common Crawl), PubMed Abstracts, Books3, OpenWebText2, ArXiv, and GitHub. 

\textbf{SlimPajama},
an open-source reproduction of the LLaMA v1 dataset with extensively deduplication. It incorporates data from CommonCrawl (CC), C4, GitHub, Books, ArXiv, Wikipedia, and StackExchange, totaling 627 billion tokens \citep{cerebras2023slimpajama}.
...
For instance, GPT-3 was trained on a filtered Common Crawl dataset and Wikipedia \citep{gpt3}, which are represented by C4 and WikiText-103. It was also trained on WebText2 and Books, which are sub-datasets of The Pile.

We also evaluate the diversity coefficient of the following six sub-datasets of The Pile:

\textbf{Pile-CC}, a 227 GiB preprocessed version of Common Crawl's web crawl corpus \citep{thepile}. While both Pile-CC and C4 are sourced from Common Crawl, Pile-CC was preprocessed from Web Archive files, which are raw HTTP responses and page HTML, whereas C4 was preprocessed from WET files, which consist of plaintext. Nonetheless, we expect that both datasets are non-mutually-exclusive.

\textbf{HackerNews}, a 4 GiB scraped and parsed dataset of comment trees from Hacker News, a social news website that aggregates article links \citep{thepile}. Articles are generally focused on topics in computer science and entrepreneurship.

\textbf{NIH ExPorter}, a 1.9 GiB dataset of NIH Grant abstracts for awarded applications from 1985-present hosted on the ExPORTER initiative \citep{thepile}.

\textbf{PubMed Abstracts}, a 19 GiB dataset of abstracts from 30 million publications in PubMed \citep{thepile}.

\textbf{USPTO Backgrounds}, a 23 GiB dataset of background sections from patents granted by the United States Patent and Trademark Office (USPTO) \citep{thepile}.
...
\begin{figure*}[h]
    \centering \includegraphics[width=0.5\columnwidth]{plots/truth_vs_task2vec.png}
    \caption{\textbf{Task2Vec diversity coefficient correlates with ground truth diversity for synthetic Gaussian benchmark.} Source: \citep{diversity}}
\end{figure*}

As shown in \citep{diversity}, when the ground truth diversity is available for a synthetic Gaussian benchmark, the Task2Vec diversity coefficient correlates with the ground truth diversity. These results provide confidence in the Task2Vec diversity coefficient as diversity metric.
...
% \end{itemize}
Since using a random and/or non fine-tuned network is more resource efficient and easily accessible in practice, our motivation is to assess the necessity of using pre-trained and fine-tuned probe network, which is the original configuration used for Task2Vec in \citep{task2vec}. 
...
% \begin{itemize}
    ...
    Figure~\ref{fig:batchsize_network_experiments} (left) shows that using a random probe network underestimates diversity compared to pretrained networks, which is in accordance with results from \citep{diversity} on vision datasets and indicative of random networks' estimates diverging from pre-trained networks'. 
    ...
    % Lightly fine-tuning (the final layer \citep{task2vec}) of the probe network ensures the final Task2Vec embedding is more faithful to the dataset in question, as it adjusts the batch/task representation to a more similar distribution.
...
Since the other setting's diversity difference is large from the ground truth \citep{task2vec}, we can't recommend it. 
...
This is in fact the same argument from \citep{curse_low_div}---but we verified it applied in our setting.
...
We base this inference on the fact that the manifestation of emergence is intimately tied to the specific metric employed, and the sudden unpredictable jumps disappear when smooth metrics are applied \citep{schaeffer2023emergent}.
...
In contrast, the diversity coefficient is well-justified, extensively tested in our work and previous work, e.g. the diversity coefficient correlates with ground truth diversities, cluster according to semantics, taxonomy etc. (see Section~\ref{appendix:ground_truth_div} and \citep{task2vec, curse_low_div}). 
...
Data has taken a central role in the success of modern machine learning methods, like GPT4 \citep{gpt4}, CLIP \citep{clip}, and PaLM 2 \citep{google2023palm2}, with special relevance for architectures with few inductive biases, like the popular Transformer \citep{vaswani2017attention}.
...
% However, this type of experiment is challenging because pre-training an LLM at scale is expensive---more than 10M dollars \citep{ruiz2023train}.
