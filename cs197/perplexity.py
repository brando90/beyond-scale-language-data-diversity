...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(model_args.model_dir)
    n_params = sum({p.data_ptr(): p.numel() for p in model.parameters()}.values())
    print('model param count:', n_params)

    # model.load_state_dict(
    #     torch.load(model_args.model_dir + '/model.safetensors', map_location='cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    eval_dataset = random.sample(sorted(eval_dataset), max_eval_samples)
